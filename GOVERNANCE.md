...
`cloud-itonami-3312` is an OSS open-business blueprint for community
industrial machinery repair operations, robotics-premised.

## Maintainers
Maintainers may merge changes that preserve these invariants:
- a robot action the governor refuses is never dispatched to hardware.
- the Machinery Repair Governor remains independent of the advisor.
- hard policy violations (a return-to-service release without a
  completed post-repair inspection, a repair step outside verified
  repair-authority scope) cannot be overridden by human approval.
